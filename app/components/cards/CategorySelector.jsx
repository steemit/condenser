...
export function validateCategory(category, required = true) {
    ...
    return (
        ...
        cats.find(c => /[ҐЄІЇA-ZА-ЯЁ]/.test(c))      ? translate('use_only_lowercase_letters') :
        // check for russian or english symbols
        cats.find(c => !/^[ґєіїa-zа-яё0-9-]+$/.test(c)) ? translate('use_only_allowed_characters') :
        cats.find(c => !/^[ґєіїa-zа-яё-]/.test(c)) ? translate('must_start_with_a_letter') :
        cats.find(c => !/[ґєіїa-zа-яё0-9]$/.test(c)) ? translate('must_end_with_a_letter_or_number') :
        null
    )
}

...
export default class LocalizedCurrency extends Component {
    static propTypes = {
    ...
    componentDidMount() {
        this.setState({
            xchangePicked: this.props.currency
                ? this.props.currency
                : localStorage.getItem('xchange.picked') || DEBT_TOKEN_SHORT,
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (fetching !== nextProps.fetching) {
            const xchangePicked = this.props.currency
                ? this.props.currency
                : localStorage.getItem('xchange.picked') || DEBT_TOKEN_SHORT;
    ...
    render() {
        ...
        if (!process.env.BROWSER || fetching || xchangePair === 0 || xchangeGold === 0)
        ...
        localizedCurrency = (number, options) => {
            ...
            if (process.env.BROWSER) {
                ...
                if (options && options.maximumFractionDigits) {
                    ...
                    if (!nRounding) {
                        if (vesting_shares < 10000000 && vesting_shares != 0)
                            //FIXME this is vesting_shares param. Move to config file needed
                            nRounding = FRACTION_DIGITS_MARKET;
                        else nRounding = FRACTION_DIGITS;
                    }
                }
            }
            ...
            if (options ? (options.short ? options.short : short) : short) {
                ...
                if (options ? (options.rounding ? options.rounding : rounding) : rounding) {
                    let divider = Math.pow(
                        10,
                        parseInt(Math.ceil(currencyAmount).toString().length) - 1
                    );

...
function validatePassword(value, new_password) {
    ...
    const length = 32;
    return value && value.length < length ? tt('password_must_be_characters_or_more', {amount: length}) : '';
}
...
export default class PasswordInput extends React.Component {
    ...
    newPasswordChange(e) {
        ...
        if (value !== this.state.confirmPassword.value) {
            ...
            res.confirmPassword.error = tt('passwords_do_not_match');
        }
        this.setState(res);
        this.onChange(res);
    }
    ...
    confirmPasswordChange(e) {
        const value = e.target.value.trim();
        let error = '';
        if (value !== this.state.newPassword.value) error = tt('passwords_do_not_match');

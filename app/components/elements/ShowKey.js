...
class ShowKey extends Component {
    ...
    render() {
        ...
        const keyIcon = <span style={{ fontSize: "100%" }}>{
            translate("hide_private_key")
        }</span>;
        // Tooltip is trigggering a setState on unmounted component exception
        const showTip = translate("show_private_key");
        //<Tooltip t="Show private key (WIF)">show</Tooltip>
        const keyLink = wif
            ? <div style={
                { marginBottom: 0 }
            } className="hollow tiny button slim"><a onClick={onShow}>{
            ...
            : authType === "memo"
                ...
                : authType === "owner"
                    ...
                    : <div style={
                        { marginBottom: 0 }
                    } className="hollow tiny button slim"><a onClick={
                        showLogin
                    }>{translate("login_to_show")}</a></div>;
...
export default connect(
    ...
    dispatch => ({
        showLogin: ({ username, authType }) => {
            dispatch(
                user.actions.showLogin({ loginDefault: { username, authType } })
            );
        }
    })
)(ShowKey)
    ...
    render() {
            ...
            return <span key={idx++}>
                ...
                <div className="row">
                    <div className="column small-2">
                        <label>{tt('public')}</label>
                        <QRCode text={keyObj.pubkey} />
                    </div>
                    <div className="column small-8">
                        <label>{tt('public_something_key', {key})}</label>
                        <div className="overflow-ellipsis">
                            <code><small>{keyObj.pubkey}</small></code>
                        </div>
                        {keyObj.wif && <div>
                            <label>{tt('private_something_key', {key})}</label>
                            <div className="overflow-ellipsis">
        ...
        return (<div className="row">
            <div style={{paddingBottom: 10}} className="column small-12">
                ...
                <span className="secondary">{tt('posting_key_is_required_it_should_be_different')}</span>
            </div>
            <div style={{paddingBottom: 10}} className="column small-12">
                <Keys account={account} authType="active" onKey={onKey.Active} />
                <span className="secondary">{tt('the_active_key_is_used_to_make_transfers_and_place_orders')}</span>
            </div>
            <div style={{paddingBottom: 10}} className="column small-12">
                <Keys account={account} authType="owner" onKey={onKey.Owner} />
                <span className="secondary">{tt('the_owner_key_is_required_to_change_other_keys')}<br />{tt('the_private_key_or_password_should_be_kept_offline')}</span>
            </div>
            <div style={{paddingBottom: 10}} className="column small-12">
                <Keys account={account} authType="memo" onKey={onKey.Memo} />
                <span className="secondary">{tt('the_memo_key_is_used_to_create_and_read_memos')}</span>
            </div>
            {/*

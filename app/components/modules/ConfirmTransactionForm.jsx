    ...
    render() {
        ...
        return (
           <div className="ConfirmTransactionForm">
               ...
               <br />
               <button className="button" onClick={okClick}>{tt('ok')}</button>
               <button type="button hollow" className="button hollow" onClick={onCancel}>{tt('cancel')}</button>
           </div>
       )
    }
}
const typeName = confirmBroadcastOperation => {
    ...
    return tt('confirm') + ' ' + (type.split('_').map(n => n.charAt(0).toUpperCase() + n.substring(1))).join(' ')
}

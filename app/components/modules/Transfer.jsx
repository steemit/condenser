...
class TransferForm extends Component {
    ...
    initForm(props) {
        ...
        reactForm({
            ...
            validation: values => ({
                to: !values.to ? tt('g.required') : validate_account_name(values.to),
                amount:
                    !parseFloat(values.amount) || /^0$/.test(values.amount)
                        ? tt('g.required')
                        : insufficientFunds(values.asset, values.amount)
                            ...
                            : countDecimals(values.amount) > 3
                                ...
                                : null,
                asset: props.toVesting ? null : !values.asset ? tt('g.required') : null,
                memo:
                    values.memo && (!browserTests.memo_encryption && /^#/.test(values.memo))
                        ...
                        : checkMemo(values.memo)
                            ? tt('transfer_jsx.private_key_in_memo')
                            : null,
            }),
        });
    }
    ...
    balanceValue() {
        ...
        return !asset || asset.value === 'GOLOS'
            ? isWithdraw
                ? currentAccount.get('savings_balance')
                : currentAccount.get('balance')
            : asset.value === 'GBG'
    ...
    render() {
        ...
        const powerTip2 = tt(
            'tips_js.VESTING_TOKEN_is_non_transferrable_and_requires_convert_back_to_LIQUID_TOKEN',
            { LIQUID_TOKEN: LIQUID_TICKER, VESTING_TOKEN2 }
        );
        const powerTip3 = tt(
        ...
        const form = (
            ...
            >
                ...
                {!toVesting && (
                    <div>
                        <div className="row">
                            <div className="column small-12">{transferTips[transferType]}</div>
                        </div>
                        <br />
                    </div>
                )}
                ...
                {memo &&
                    !disableMemo && (
                        <div className="row">
                            <div className="column small-2" style={{ paddingTop: 33 }}>
                                {tt('transfer_jsx.memo')}
                            </div>
                            <div className="column small-10">
                                <small>
                                    {tt('transfer_jsx.this_memo_is') + isMemoPrivate
                                        ? tt('transfer_jsx.public')
                                        : tt('transfer_jsx.private')}
                                </small>
                                <input
                                    type="text"
                ...
                {!loading && (
                    <span>
                        ...
                        {transferToSelf && (
                            ...
                            >
                                {tt(advanced ? 'g.basic' : 'g.advanced')}
                            </button>
                        )}
                    </span>
                )}
...
const AssetBalance = ({ onClick, balanceValue }) => (
    <a onClick={onClick} style={{ borderBottom: '#A09F9F 1px dotted', cursor: 'pointer' }}>
...
export default connect(
    (state, props) => {
        ...
        const currentAccount =
            currentUser && state.global.getIn(['accounts', currentUser.get('username')]);

        if (!toVesting && !initialValues.transferType)
            initialValues.transferType = 'Transfer to Account';

        let transferToSelf =
            toVesting || /Transfer to Savings|Savings Withdraw/.test(initialValues.transferType);
    ...
    dispatch => ({
        dispatchSubmit: ({
            ...
            if (
                !toVesting &&
                !/Transfer to Account|Transfer to Savings|Savings Withdraw/.test(transferType)
            ) {
                throw new Error(

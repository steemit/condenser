...
const ru = {
    ...
    we_require_social_account: APP_NAME +
        " спонсирует каждый аккаунт на сумму около {signup_bonus} в " +
        VESTING_TOKEN +
        "; чтобы предотвратить злоупотребление, мы просим новых пользователей регистрироваться используя социальные сети.",
    ...
    by_verifying_you_agree: "Подтверждая ваш аккаунт вы соглашаетесь с условиями проекта " +
        APP_NAME,
    ...
    and: "и",
    are_well_tested_and_known_to_work_with: "хорошо тестированы и работают с " +
        APP_URL +
        ".",
    ...
    get_VESTING_TOKEN_when_sign_up: "Получи {signupBonus} " + VESTING_TOKEN +
        " подписавшись сегодня.",
    ...
    // NOTE this might be deleted in future
    ...
    // example: 'deposit using Steem Power' // TODO: is this example right?
    ...
    // PostFull.jsx
    by: " ",
    // здеcь специально ничего нет, пример: "posted by 'Vitya'" > "добавил 'Vitya'"
    in: "в",
    share: "Поделиться",
    ...
    transfer_amount_VESTING_TOKEN_to: "Перевод {amount} " + VESTING_TOKEN +
        " в",
    recieve_amount_VESTING_TOKEN_from: "Получение {amount} " + VESTING_TOKEN +
        " от",
    transfer_amount_VESTING_TOKEN_from_to: "Передать {amount} " +
        VESTING_TOKEN +
        " от {from} к",
    ...
    transfer_amount_steem_power_to: "Перевод {amount} " + LIQUID_TICKER +
        " в Силу Голоса",
    stop_power_down: "Ослабление Силы Голоса остановлено",
    start_power_down_of: "Ослабление Силы Голоса начато с",
    curation_reward_of_VESTING_TOKEN_for: "Кураторские вознаграждения {reward} " +
        VESTING_TOKEN +
        " за",
    author_reward_of_VESTING_TOKEN_for: "Авторские вознаграждения {payout} и {reward} " +
        VESTING_TOKEN +
        " за",
    ...
    // this probably will end with question mark
    ...
    the_rules_of_APP_NAME: "Первое правило сети " + APP_NAME +
        ": не теряйте свой пароль. <br /> Второе правило " +
        APP_NAME +
        ": <strong>Не</strong> теряйте свой пароль. <br /> Третье правило " +
        APP_NAME +
        ": мы не можем восстановить ваш пароль. <br /> Четвертое правило: если вы можете запомнить свой пароль, значит он не безопасен. <br /> Пятое правило: используйте только сгенерированные случайным образом пароли. <br /> Шестое правило: Никому не говорите свой пароль. <br /> Седьмое правило: Всегда надежно храните свой пароль.",
    ...
    new: "Новое",
    // ex. 'Generated Password (new)', but not exclusively
    age: "Новое",
    votes: "votes",
    ...
    understand_that_APP_NAME_cannot_recover_password: "Я понимаю что " +
        APP_NAME +
        " не сможет восстановить утраченный пароль",
    ...
    ok: "Ок",
    // Лучше использовать "хорошо" или "ладно"?
    convert_to_VESTING_TOKEN: "Перевести в " + VESTING_TOKEN,
    DEBT_TOKEN_will_be_unavailable: "Эта операция будет проходить 3,5 дней от настоящего момента и ее нельзя отменить. Эти " +
        DEBT_TOKEN +
        " мгновенно станут недоступны",
    amount: "Количество",
    convert: "Конвертировать",
    invalid_amount: "Неверное количество",
    insufficent_balance: "Недостаточный баланс",
    in_week_convert_DEBT_TOKEN_to_LIQUID_TOKEN: "В 3,5 дня перевести {amount} " +
        DEBT_TOKEN +
        " в " +
        LIQUID_TOKEN,
    order_placed: "Заказ размещен",
    // ex.: "Order placed: Sell {someamount_to_sell} for atleast {min_to_receive}"
    follow: "Подписаться",
    unfollow: "Отписаться",
    mute: "Блокировать",
    ...
    post: "Пост",
    // places used: tooltip in MediumEditor
    unknown: "Неизвестный",
    // exp.: 'unknown error'
    ...
    recover_account_intro: "Иногда бывает что  ключ владельца может быть скомпрометирован. Восстановление украденного аккаунта дает законному владельцу 30 дней чтобы вернуть аккаунт с момента изменения владельческого ключа мошенником. Восстановление украденного аккаунта в " +
        APP_URL +
        " возможно только если владелец аккаунта ранее указал '" +
        APP_NAME +
        "' в качестве доверенного лица и согласился с Условиями Использования сайта  " +
        APP_NAME +
        ".",
    ...
    thanks_for_submitting_request_for_account_recovery: "<p>Благодарим Вас за отправку запроса на восстановление аккаунта используя основанную на блокчейне мультифакторную аутентификацию " +
        APP_NAME +
        "’a.</p> <p>Мы ответим Вам как можно быстрее, однако, пожалуйста, ожидайте что может быть некоторая задержка из-за большого объема писем.</p> <p>Пожалуйста, будьте готовы подтвердить свою личность.</p>",
    ...
    vote: "Проголосовать",
    // context: to vote? (title attribute on voting button)
    witness: "Делегаты",
    ...
    APP_NAME_can_only_register_one_account_per_verified_user: "Пожалуйста, имейте ввиду что " +
        APP_NAME +
        " может регистрировть только один аккаунт для каждого подтвержденного пользователя",
    ...
    our_records_indicate_you_already_have_account: "Наши записи показывают что у вас уже есть " +
        APP_NAME +
        " аккант",
    to_prevent_abuse_APP_NAME_can_only_register_one_account_per_user: "Чтобы предотвратить злоупотребление (каждый зарегистрированный аккаунт стоит {amount} в " +
        LIQUID_TOKEN +
        ") " +
        APP_NAME +
        " может регистрировать только один аккаунт для каждого подтвержденного пользователя.",
    // next 3 blocks are meant to be used together
    you_can_either: "Вы можете или",
    // context 'you can either login'
    to_your_existing_account_or: "в ваш существующий аккаунт или",
    // context: 'to your existing account or send us email'
    ...
    your_existing_DEBT_TOKEN_are_liquid_and_transferable: "Ваши существующие " +
        DEBT_TOKEN +
        " ликвидны и перемещаемы. Возможно, вы хотите торговать " +
        DEBT_TOKEN +
        " напрямую на этом сайте в разделе {link} или перевести на внешний рынок.",
    ...
    APP_NAME_cannot_recover_passwords_keep_this_page_in_a_secure_location: APP_NAME +
        " не может восстановить пароли. Сохраните эту страницу в безопасном месте, например, в огнестойком сейфе или в депозитарной ячейке.",
    APP_NAME_password_backup: APP_NAME + " резервное копирование пароля",
    APP_NAME_password_backup_required: APP_NAME +
        " резервное копирование пароля (обязательно)",
    ...
    thank_you_for_being_an_early_visitor_to_APP_NAME: "Благодарим вас за то что являетесь ранним посетителем " +
        APP_NAME +
    ...
    buy_LIQUID_TOKEN_or_VESTING_TOKEN: "Купить " + LIQUID_TOKEN + " или " +
        VESTING_TOKEN,
    // TODO
    version: "Версия",
    about_APP_NAME: "О " + APP_NAME,
    APP_NAME_is_a_social_media_platform_where_everyone_gets_paid_for_creating_and_curating_content: APP_NAME +
        " это социальная медиа платформа в которой <strong>все</strong>&nbsp;получают <strong>деньги</strong> за создание и курирование контента",
    APP_NAME_is_a_social_media_platform_where_everyone_gets_paid: APP_NAME +
        " это социальная медиа платформа в которой каждый зарабатывает за создание и курирование контента. Он использует надежную систему цифровых очков под названием " +
        LIQUID_TOKEN +
        ", который поддерживает реальную ценность для цифровых наград через выявление рыночной цены и ликвидности.",
    ...
    LIQUID_TOKEN_can_be_converted_to_VESTING_TOKEN_in_a_process_called_powering_up: LIQUID_TOKEN +
        " может быть конвертирован в " +
        VESTING_TOKEN +
        ", этот процесс называется \"усиление голоса\".",
    tokens_worth_about_AMOUNT_of_LIQUID_TOKEN: "Перемещаемые цифровые токены, цена которых всегда равна ~1 мг золота в " +
        LIQUID_TOKEN +
        ".",
    ...
    the_estimated_value_is_based_on_an_average_value_of_steem_in_US_dollars: "Сметная стоимость основана на среднем значении " +
        LIQUID_TOKEN +
        " в долларах США",
    VESTING_TOKEN_is_non_transferrable_and_requires_convert_back_to_LIQUID_TOKEN: VESTING_TOKEN +
        " нельзя передавать и потребуется 3 месяца и 13 выплаты чтобы перевести обратно в " +
        LIQUID_TOKEN +
        ".",
    // TODO
    converted_VESTING_TOKEN_can_be_sent_to_yourself_but_can_not_transfer_again: "Конвертированная " +
        VESTING_TOKEN +
        " может быть отправлена себе или кому-то еще, но не может быть передана вновь без конвертирования назад в " +
        LIQUID_TOKEN +
        ".",
    profile: "Профиль",
    ...
    spend_your_DEBT_TOKEN_to_advertise_this_post: "Используйте ваши " +
        DEBT_TOKEN +
        " чтобы прорекламировать этот пост в секции продвигаемого контента",
    ...
    by_verifying_you_agree_with_privacy_policy_of_website_APP_URL: "сайта " +
        APP_URL,

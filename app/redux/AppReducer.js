...
export default function reducer(state = defaultState, action) {
    ...
    if (action.type === "WS_CONNECTION_STATUS") {
        return state.updateIn([ "ws_connection" ], value => {
            if (value && value.status === action.payload.status)
                return value;
    ...
    if (action.type === "RPC_REQUEST_STATUS") {
        ...
        const loadingIgnored = loadingBlacklist.indexOf(
            action.payload.method
        ) !==
            -1;
        if (action.payload.event === "BEGIN") {
            res = state.mergeDeep({
                loading: loadingIgnored ? state.get("loading") : true,
                // reuse current loading state if the method is blacklisted
                requests: { [request_id]: Date.now() },
                ignoredLoadingRequestCount: state.get(
        ...
        if (
            action.payload.event === "END" || action.payload.event === "ERROR"
        ) {
            const ignoredLoadingRequestCount = state.get(
                "ignoredLoadingRequestCount"
            ) -
                (loadingIgnored ? 1 : 0);
            res = res.deleteIn([ "requests", request_id ]);
            const loading = res.get("requests").size -
                ignoredLoadingRequestCount >
                0;
            res = res.mergeDeep({ loading, ignoredLoadingRequestCount });
        }
    }

...
export function* loadFollows(method, account, type, force = false) {
    ...
    if (!force) {
        const hasResult = yield select(
            state => state.global.hasIn([
                "follow",
                method,
                account,
                type + "_result"
...
function* loadFollowsLoop(method, account, type, start = "", limit = 100) {
    const res = fromJS(
        (yield Apis.follow(method, account, start, type, limit))
    );
        ...
        payload: {
            ...
            updater: m => {
                ...
                res.forEach(value => {
                    ...
                    const accountNameKey = method === "get_following"
                        ? "following"
                        : "follower";

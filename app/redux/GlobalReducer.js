...
export default createModule({
    ...
    transformations: [
            ...
            reducer: (state, action) => {
                return state.withMutations(map => {
                    map.updateIn([ "content", action.payload.post ], value => {
                        value.merge(
                            Map({ collapsed: action.payload.collapsed })
                        );
                    });
                });
            }
        ...
        },
        {
            ...
            reducer: (state, { payload: { account, proxy } }) => state.setIn(
                [ "accounts", account, "proxy" ],
                proxy
            )
        },
        {
        ...
        },
        {
            ...
            reducer: (
                state,
                { payload: { key, notSet = Map(), updater } }
        ...
        },
        {
            ...
            reducer: (state, { payload: { id, result, error } }) => state.set(
                id,
                fromJS({ result, error })
            )
        },
        {
            ...
            reducer: (
                ...
                "active_dialogs",
                Map(),
                d => d.set(name, fromJS({ params }))
            )
        },
        {
            action: "HIDE_DIALOG",
            reducer: (state, { payload: { name } }) => state.update(
                "active_dialogs",
                d => d.delete(name)
            )
        }

...
export const sharedWatches = [
    watchGetState,
    watchWsConnectionStatus,
    watchTransactionErrors
];

export function* getAccount(username, force = false) {
    let account = yield select(
        state => state.global.get("accounts").get(username)
    );
    if (force || !account) {
    ...
    if (notifications && notifications.has("ws:connection:error")) {
        if (status === "open") {
            yield put({
                ...
                payload: { key: "ws:connection:error" }
            });
        }
    } else if (status !== "open") {
        yield call(wait, 3000);
        const ws_connection = yield select(
            state => state.app.get("ws_connection")
        );

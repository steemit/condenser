...
function* preBroadcast_transfer({ operation }) {
    ...
    if (memo) {
        ...
        if (/^#/.test(memo)) {
            ...
            if (!memo_private)
                throw new Error(
                    "Unable to encrypte memo, missing memo private key"
                );
...
const toStringUtf8 = o => o
    ? Buffer.isBuffer(o) ? o.toString("utf-8") : o.toString()
    : o;
...
function* preBroadcast_account_witness_vote({ operation, username }) {
    ...
    yield put(
        g.actions.updateAccountWitnessVote({ account, witness, approve })
    );
    return operation;
}
...
function* error_account_witness_vote(
    ...
    yield put(
        g.actions.updateAccountWitnessVote({
            account,
            witness,
            approve: !approve
        })
...
function* broadcast(
    ...
    try {
        yield new Promise((resolve, reject) => {
            if (process.env.BROWSER && window.bump === 1) {
                // for testing
                console.log(
                    "TransactionSaga bump(no broadcast) and reject",
                    transaction.toObject(tx)
                );
                setTimeout(
                    () => {
            ...
            } else if (process.env.BROWSER && window.bump === 2) {
                // also for testing
                console.log(
...
function* accepted_withdraw_vesting({ operation }) {
    let [ account ] = yield call(Apis.db_api, "get_accounts", [
        operation.account
    ]);
...
function* recoverAccount(
    ...
    const [ account ] = yield call([ Apis, Apis.db_api ], "get_accounts", [
        account_to_recover
    ]);
    if (!account) {
    ...
    try {
        ...
        oldOwner = PrivateKey.fromSeed(
            account_to_recover + "owner" + old_password
        );
    }
    const newOwner = PrivateKey.fromSeed(
        account_to_recover + "owner" + new_password.trim()
    );
    const newActive = PrivateKey.fromSeed(
        account_to_recover + "active" + new_password.trim()
    );
    const newPosting = PrivateKey.fromSeed(
        account_to_recover + "posting" + new_password.trim()
    );
    const newMemo = PrivateKey.fromSeed(
        account_to_recover + "memo" + new_password.trim()
    );
    ...
    try {
        ...
        yield new Promise(
            (resolve, reject) => Apis.broadcastTransaction(sx1, () => {
                resolve();
            }).catch(e => {
                reject(e);
            })
            ...
            [
                "account_update",
                {
                    ...
                    active: {
                        weight_threshold: 1,
                        account_auths: [],
                        key_auths: [ [ newActive.toPublicKey(), 1 ] ]
                    },
                    posting: {
                        weight_threshold: 1,
                        account_auths: [],
                        key_auths: [ [ newPosting.toPublicKey(), 1 ] ]
                    },
        ...
        yield new Promise(
            (resolve, reject) => Apis.broadcastTransaction(sx2, () => {
                resolve();
            }).catch(e => {
                reject(e);
            })
...
function* updateAuthorities(
    ...
    const payload = {
        type: "account_update",
        operation: { account: account.name, ...ops2, memo_key, json_metadata },
        keys: [ key ],
...
function* updateMeta(params) {
    ...
    try {
        ...
        const tx = yield createTransaction([
            [
                "update_account_meta",
                { account_name: account.name, json_meta: JSON.stringify(meta) }
            ]
        ]);

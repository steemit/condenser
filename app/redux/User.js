...
export default createModule({
    ...
    transformations: [
        ...
        },
        {
            ...
            reducer: (state, { payload }) => state.set(
                "saveLoginConfirm",
                payload
            )
        },
        { action: "SAVE_LOGIN", reducer: state => state },
        // Use only for low security keys (like posting only keys)
        {
            action: "REMOVE_HIGH_SECURITY_KEYS",
            reducer: state => {
                if (!state.hasIn([ "current", "private_keys" ]))
                    return state;
                let empty = false;
        ...
        {
            action: "SHOW_TRANSFER",
            reducer: state => state.set("show_transfer_modal", true)
        },
        {
            action: "HIDE_TRANSFER",
            reducer: state => state.set("show_transfer_modal", false)
        },
        {
        ...
        {
            action: "SHOW_SIGN_UP",
            reducer: state => state.set("show_signup_modal", true)
        },
        {
            action: "HIDE_SIGN_UP",
            reducer: state => state.set("show_signup_modal", false)
        },
        {
            action: "KEYS_ERROR",
            reducer: (state, { payload: { error } }) => state.merge({
        ...
        },
        {
            ...
            reducer: (
                state,
                { payload: { accountName, auth, pub_keys_used } }
            ) => {

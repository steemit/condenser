...
function* loadSavingsWithdraw() {
    ...
    const withdraws = List(fromJS(m).values()).sort(
        (a, b) => strCmp(a.get("complete"), b.get("complete"))
    );

    yield put(user.actions.set({ key: "savings_withdraws", value: withdraws }));
}
...
function* removeHighSecurityKeys({ payload: { pathname } }) {
    const highSecurityPage = highSecurityPages.find(p => p.test(pathname)) !=
        null;
...
function* usernamePasswordLogin2(
    ...
    // no saved password
    if (!username || !password) {
        const offchain_account = yield select(
            state => state.offchain.get("account")
        );
        if (offchain_account)
            serverApiLogout();
        return;
    ...
    const highSecurityLogin = // /owner|active/.test(userProvidedRole) ||
    // isHighSecurityOperations.indexOf(operationType) !== -1 ||
    highSecurityPages.find(p => p.test(pathname)) != null;

    const isRole = (role, fn) => !userProvidedRole || role === userProvidedRole
        ? fn()
        : undefined;
    ...
    if (!account) {
        yield put(
            user.actions.loginError({ error: "Username does not exist" })
        );
        return;
    }
    ...
    try {
        ...
        // Password (non wif)
        login_owner_pubkey = PrivateKey
            .fromSeed(username + "owner" + password)
            .toPublicKey()
            .toString();
        private_keys = fromJS({
            posting_private: isRole(
                "posting",
                () => PrivateKey.fromSeed(username + "posting" + password)
            ),
            active_private: isRole(
                "active",
    ...
    let authority = yield select(
        state => state.user.getIn([ "authority", username ])
    );
    ...
    if (!fullAuths.size) {
        ...
        if (
            ...
            yield put(
                user.actions.loginError({
                    error: "Hello. Your account may have been compromised. We are working on restoring an access to your account. Please send an email to support@steemit.com."
                })
            );
            return;
        }
        if (
            ...
            yield put(
                user.actions.loginError({ error: "owner_login_blocked" })
            );
        } else if (!highSecurityLogin && hasActiveAuth) {
            yield put(
                user.actions.loginError({ error: "active_login_blocked" })
            );
        } else {
    ...
    if (!highSecurityLogin) {
        if (
            posting_pubkey === owner_pubkey || posting_pubkey === active_pubkey
        ) {
            yield put(
                user.actions.loginError({
...
function* saveLogin_localStorage() {
    ...
    localStorage.removeItem("autopost2");
    const [ username, private_keys, login_owner_pubkey ] = yield select(
        state => [
    ...
    const account = yield select(
        state => state.global.getIn([ "accounts", username ])
    );
    if (!account) {
...
function* lookupPreviousOwnerAuthority({ payload: {} }) {
    ...
    const username = current.get("username");
    const key_auths = yield select(
        state => state.global.getIn([
            "accounts",
            username,
            "owner",
            "key_auths"
    ...
    let owner_history = fromJS(
        (yield call(Apis.db_api, "get_owner_history", username))
    );
    ...
    const previous_owner_authority = owner_history.find(o => {
        ...
        const key3 = auth
            .get("key_auths")
            .find(
                key2 => key2.get(0) === login_owner_pubkey &&

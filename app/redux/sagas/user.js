...
function* removeHighSecurityKeys({ payload: { pathname } }) {
    ...
    // disappears again).
    if (!highSecurityPage) yield put(user.actions.removeHighSecurityKeys());
}
...
function* usernamePasswordLogin(action) {
    ...
    if (externalTransferRequested) {
        if (offchain_account) {
            if (offchain_account !== sender) preventLogin = true;
        }
    }
...
const clean = value =>
    value == null || value === '' || /null|undefined/.test(value) ? undefined : value;
...
function* usernamePasswordLogin2({
    ...
    if (!username && !password) {
        ...
        if (data) {
            ...
            [username, password, memoWif, login_owner_pubkey] = new Buffer(data, 'hex')
                .toString()
                .split('\t');
    ...
    yield call(accountAuthLookup, {
        payload: { account, private_keys, highSecurityLogin, login_owner_pubkey },
    });
...
function* saveLogin_localStorage() {
    ...
    const data = new Buffer(
        `${username}\t${posting_private.toWif()}\t${memoWif || ''}\t${login_owner_pubkey || ''}`
    ).toString('hex');

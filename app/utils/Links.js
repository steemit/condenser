...
export const local = (flags = "i") => new RegExp(
    urlSet({ domain: "(?:localhost|(?:.*\\.)?steemit.com)" }),
    flags
);
export const remote = (flags = "i") => new RegExp(
    urlSet({ domain: `(?!localhost|(?:.*\\.)?steemit.com)${domainPath}` }),
    flags
);
export const youTube = (flags = "i") => new RegExp(
    urlSet({ domain: "(?:(?:.*.)?youtube.com|youtu.be)" }),
    flags
);
export const image = (flags = "i") => new RegExp(
    urlSet({ path: imagePath }),
    flags
);

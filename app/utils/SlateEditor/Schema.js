...
export const schema = {
    ...
    // sidebarTypes: [],
    nodes: {
        "paragraph": ({ children, attributes }) => <p {...attributes}>{
            children
        }</p>,
        "code-block": ({ children, attributes }) => <pre {...attributes}><code>{
            children
        }</code></pre>,
        "block-quote": (
            { children, attributes }
        ) => <blockquote {...attributes}>{children}</blockquote>,
        "bulleted-list": ({ children, attributes }) => <ul {...attributes}>{
            children
        }</ul>,
        "numbered-list": ({ children, attributes }) => <ol {...attributes}>{
            children
        }</ol>,
        "heading-one": ({ children, attributes }) => <h1 {...attributes}>{
            children
        }</h1>,
        "heading-two": ({ children, attributes }) => <h2 {...attributes}>{
            children
        }</h2>,
        "heading-three": ({ children, attributes }) => <h3 {...attributes}>{
            children
        }</h3>,
        "heading-four": ({ children, attributes }) => <h4 {...attributes}>{
            children
        }</h4>,
        "list-item": ({ children, attributes }) => <li {...attributes}>{
            children
        }</li>,
        "table": ({ children, attributes }) => <table {...attributes}>{
            children
        }</table>,
        "thead": ({ children, attributes }) => <thead {...attributes}>{
            children
        }</thead>,
        "tbody": ({ children, attributes }) => <tbody {...attributes}>{
            children
        }</tbody>,
...
export const HtmlRules = [
    ...
    // Block rules
    {
        deserialize: (el, next) => {
            ...
            let children = code && code.tagName == "code"
                ? code.children
                : el.children;
    ...
    // Mark rules
    {
        deserialize: (el, next) => {
            ...
            if (!type)
                return;
            return { kind: "mark", type: type, nodes: next(el.children) };
        },
        serialize: (object, children) => {
            if (object.kind !== "mark")
                return;
        ...
        deserialize: (el, next) => {
            ...
            case "img":
                return {
                    ...
                    type: "image",
                    isVoid: true,
                    data: { src: el.attribs.src, alt: el.attribs.alt },
                    nodes: next(el.children)
                };
            case "a":
                return {

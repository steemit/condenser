...
/**
 * Ranked sort feed page body — legacy PostsIndex with a <sort> param.
 *
 * Shared by the /[sort] route (app/(main)/[sort]/page.tsx) and the home
 * page (/ renders the trending feed, like legacy ResolveRoute.js mapping
 * `/` → PostsIndex ['trending']). Previously /trending had a dedicated
 * static page duplicating this logic, which drifted from the [sort]
 * implementation (no NotFound handling, no case normalization).
 *
 * `sort` is the raw URL segment; membership checks and API calls use the
 * lowercased form (proxy.ts lowercases only for validation and passes the
 * raw-cased segment through, so `/Trending` must still query `trending`).
 */
export function SortFeed({ sort: sortString }: { sort: string }) {
  ...
  return (
    <FeedLayout>
      ...
      <PostsList
        ...
        // Legacy lowercases the order before it reaches PostsList
        // (server.js 301-normalizes cased sort URLs); PostSummary and the
        // empty-state keys branch on lowercase values.
        order={order}
      />
    </FeedLayout>
  );

// sometimes it's impossible to use html tags to style coin name, hence usage of _UPPERCASE modifier
export const APP_NAME = 'Golos' // 'Steemit'
export const APP_NAME_UPPERCASE = 'Golos' // 'STEEMIT'
export const APP_URL = 'golos.io' // 'steemit.com'
export const OWNERSHIP_TOKEN = 'Golos' // 'Steem'
// sometimes it's impossible to use html tags to style coin name, hence usage of _UPPERCASE modifier
export const OWNERSHIP_TOKEN_UPPERCASE = 'GOLOS' // 'STEEM'
export const INVEST_TOKEN = 'Golos Power' // 'Steem Power'
export const INVEST_TOKEN_UPPERCASE = 'GOLOS POWER' // 'STEEM POWER'
export const INVEST_TOKEN_SHORT = 'MG' // 'SP'
export const DEBT_TOKEN = 'Golos Backed Gold' // 'STEEM DOLLAR'
// export const DEBT_TOKEN_UPPERCASE = 'Golos Backed Gold' // 'STEEM DOLLAR'
export const DEBT_TOKEN_SHORT = 'GBG' // 'SD'
export const CURRENCY_SIGN = '₽' // '$'
export const LANDING_PAGE_URL = 'https://steem.io'
export const WHITEPAPER_URL = 'https://steem.io/SteemWhitePaper.pdf'
export const SEGMENT_ANALYTICS_KEY = 'F7tldQJxt491gXYqDGi5TkTT4wFpSPps'

// export const LANDING_PAGE_URL =
// export const WHITEPAPER_URL =

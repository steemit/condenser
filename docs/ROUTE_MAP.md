...
| `/category/@username/permlink` | `Post` | Rewrite → `/post/<category>/<username>/<permlink>` (proxy.ts:98-113); **no reserved-word check** — the legacy Post regex `<tag>/<account>/<permlink>` has none, so `/about/@a/p`, `/welcome/@a/p`, `/hot/@a/p` and `/tags/@user/permlink` all render Post pages (legacy static checks are exact-path, and CategoryFilters matches at most two segments) | `app/(main)/post/[category]/[username]/[permlink]/page.tsx` | Implemented |
| `/@username/feed` | `PostsIndex ['home', user]` | Rewrite → `/user/<username>/feed` (proxy.ts:115-125) | `app/(main)/user/[username]/[section]/page.tsx` (fetches `bridge.get_account_posts` with sort `feed`, like legacy `PostsIndex ['home', user]`) | Implemented |
| `/@username/<section>` | `UserProfile` | Rewrite → `/user/<username>/<section>` (proxy.ts:127-138); `section` must be in `SECTIONS` (proxy.ts:25-28) | `app/(main)/user/[username]/[section]/page.tsx` | Implemented |
| `/@username/<permlink>` | `PostNoCategory` | Rewrite → `/post-no-category/<username>/<permlink>` (proxy.ts:140-150); only when second segment is not a section | `app/(main)/post-no-category/[username]/[permlink]/page.tsx` (fetches category, redirects to `/<category>/@user/permlink`) | Implemented |
| `/@username` | `UserProfile` (blog tab) | Rewrite → `/user/<username>` (proxy.ts:152-164); reserved usernames rewrite to `/404` | `app/(main)/user/[username]/page.tsx` (client redirect to `/@<username>/blog`) | Implemented |
| `/<sort>/<tag>` | `PostsIndex [sort, tag]` | Pass-through when `sort` ∈ `SORT_TYPES` and `tag` doesn't start with `@` (proxy.ts:166-175) | `app/(main)/[sort]/[tag]/page.tsx` | Implemented |
| `/<sort>` | `PostsIndex [sort]` | Pass-through when `sort` ∈ `SORT_TYPES` (proxy.ts:177-190); literal `/404` rewrites to `/404` | `app/(main)/[sort]/page.tsx` (renders `NotFound` for invalid sorts) | Implemented |
...
| `/roles/<tag>` (e.g. `/roles/hive-123456`) | `CommunityRoles` | Pass-through (proxy.ts:91-96) | `app/(main)/roles/[tag]/page.tsx` | Implemented |
| `/<a>/<b>/<c>` without `@` (e.g. `/bitcoin/alice/my-post`) | `NotFound` | Rewrite → `/404` (proxy.ts:194-201), unless first segment is reserved or second starts with `@` | `app/(main)/404/page.tsx` | Implemented |
| `/<a>/<b>` without `@`, non-sort (e.g. `/alice/my-post`) | `NotFound` | Rewrite → `/404` (proxy.ts:203-210) | `app/(main)/404/page.tsx` | Implemented |
| `/<segment>` without `@`, non-sort, non-reserved (e.g. `/alice`) | `NotFound` | Rewrite → `/404` (proxy.ts:212-219) | `app/(main)/404/page.tsx` | Implemented |
| `/%40username/...` | (same as `@` variants) | `%40` is decoded to `@` before matching (proxy.ts:45-53) | same as the corresponding `@` routes | Implemented |

`SORT_TYPES` (proxy.ts:31-33): `hot`, `trending`, `promoted`, `payout`,
`payout_comments`, `muted`, `created` — identical to the legacy `<sort>`
regex alternation.

`SECTIONS` (proxy.ts:25-28): `blog`, `posts`, `comments`, `replies`,
...
A single guard in `proxy.ts` (proxy.ts:69-79) covers all four families —
...
`RESERVED_ROUTES` (proxy.ts:14-22) guards against reserved words being
...
| `/404` | Explicitly skipped (proxy.ts:59-67) | `app/(main)/404/page.tsx` | Implemented (proxy 404 target) |
| `/api/*`, `/_next/*`, `/static/*`, any path containing `.` | Skipped by proxy (proxy.ts:59-67); also excluded by the `matcher` (proxy.ts:224-235) | `app/api/**`, `app/.well-known/**`, `public/**` | Implemented |

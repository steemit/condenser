...
(function(mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        ...
        // AMD
        define(['../../lib/codemirror', 'diff_match_patch'], mod); // Plain browser env
    else mod(CodeMirror, diff_match_patch);
    ...
    function syncScroll(dv, type) {
        ...
        if (
            ...
            (editor.state.scrollSetAt || 0) + 50 > now
        )
            return false;
        ...
        if (dv.mv.options.connect == 'align') {
            ...
            var around = chunkBoundariesAround(
                dv.chunks,
                mid,
                type == DIFF_INSERT
            );
            var off = getOffsets(
                editor,
                type == DIFF_INSERT ? around.edit : around.orig
            );
            var offOther = getOffsets(
                other,
                type == DIFF_INSERT ? around.orig : around.edit
            );
        ...
        function markChunk(start, end) {
            ...
            if (start == end && bfrom == end && bto == end) {
                if (bfrom)
                    ...
                    marks.push(
                        editor.addLineClass(bfrom, 'background', classes.start)
                    );
            }
        }
    ...
    function findAlignedLines(dv, other) {
        ...
        for (var i = 0; i < dv.chunks.length; i++) {
            ...
            linesToAlign.push([
                chunk.origTo,
                chunk.editTo,
                other ? getMatchingOrigLine(chunk.editTo, other.chunks) : null,
            ]);
        }
        if (other) {
            for (var i = 0; i < other.chunks.length; i++) {
                ...
                if (j > -1)
                    linesToAlign.splice(j - 1, 0, [
                        getMatchingOrigLine(chunk.editTo, dv.chunks),
                        chunk.editTo,
                        chunk.origTo,
                    ]);
            }
    ...
    function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
        ...
        if (dv.copyButtons) {
            ...
            if (editOriginals) {
                var topReverse =
                    dv.orig.heightAtLine(chunk.editFrom, 'local') - sTopEdit;
    ...
    function copyChunk(dv, to, from, chunk) {
        ...
        to.replaceRange(
            from.getRange(Pos(chunk.origFrom, 0), Pos(chunk.origTo, 0)),
    ...
    var MergeView = (CodeMirror.MergeView = function(node, options) {
        ...
        var origLeft = options.origLeft,
            origRight =
                options.origRight == null ? options.orig : options.origRight;
    ...
    function unclearNearChunks(dv, margin, off, clear) {
        for (var i = 0; i < dv.chunks.length; i++) {
            var chunk = dv.chunks[i];
            for (
                var l = chunk.editFrom - margin;
                l < chunk.editTo + margin;
                l++
            ) {
                var pos = l + off;

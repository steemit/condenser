...
(function(mod) {
    ...
    var presto_version =
        presto && navigator.userAgent.match(/Version\/(\d*\.\d*)/);
    if (presto_version) {
    ...
    function applyTextInput(cm, inserted, deleted, sel) {
        ...
        if (cm.state.pasteIncoming && sel.ranges.length > 1) {
            if (lastCopied && lastCopied.join('\n') == inserted) {
                multiPaste =
                    sel.ranges.length % lastCopied.length == 0 &&
                    ...
                    if (mode.electricInput) {
                        if (
                            mode.electricInput.test(
                                getLine(doc, end.line).text.slice(0, end.ch)
                            )
                        ) {
                            indentLine(cm, end.line, 'smart');
                        }
            ...
            for (var i = 0; i < 4; i++) {
                ...
                if (
                    ...
                    if (ie && cm.options.lineWrapping) {
                        ...
                        if (rects.length) {
                            rect =
                                rects[bias == 'right' ? rects.length - 1 : 0];
                        } else {
                            rect = nullRect;
                        }
                    } else {
    ...
    option(
        ...
        /[\t\u0000-\u0019\u00ad\u200b-\u200f\u2028\u2029\ufeff]/g,
        function(cm, val) {
            cm.options.specialChars = new RegExp(
                val.source + (val.test('\t') ? '' : '|\t'),
                'g'
            );
            cm.refresh();
            ...
            for (var i = 0; i < first.length; ++i) {
                ...
                if (span.to == null) {
                    ...
                    if (!found) {
                        ...
                        if (sameLine) {
                            span.to =
                                found.to == null ? null : found.to + offset;
                        }
                    }
                }
            }
    ...
    function widgetHeight(widget) {
        ...
        if (!contains(document.body, widget.node)) {
            ...
            if (widget.noHScroll) {
                parentStyle +=
                    'width: ' + widget.cm.display.wrapper.clientWidth + 'px;';
            }
            removeChildrenAndAdd(

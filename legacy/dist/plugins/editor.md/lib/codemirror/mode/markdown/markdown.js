        ...
        function(cmCfg, modeCfg) {
            ...
            function blockNormal(stream, state) {
                ...
                if (state.indentationDiff >= 4) {
                    ...
                    state.prevLineHasContent &&
                    (match = stream.match(setextHeaderRE))
                ) {
                    ...
                    (stream.match(ulRE, false) || stream.match(olRE, false))
                ) {
                    var listType = null;
            ...
            function inlineNormal(stream, state) {
                ...
                if (ch === '<' && stream.match(/^\w/, false)) {
                    if (stream.string.indexOf('>') != -1) {
                        var atts = stream.string.substring(
                            1,
            ...
            function linkHref(stream, state) {
                ...
                var ch = stream.next();
                if (ch === '(' || ch === '[') {
                    state.f = state.inline = getLinkHrefInside(
                        ch === '(' ? ')' : ']'
                    );
                    if (modeCfg.highlightFormatting)
            ...
            var mode = {
                ...
                copyState: function(s) {
                    return {
                        ...
                        htmlState:
                            s.htmlState &&
                            CodeMirror.copyState(htmlMode, s.htmlState),
                ...
                token: function(stream, state) {
                    ...
                    if (stream.sol()) {
                        ...
                        var indentation = stream
                            ...
                            .replace(/\t/g, '    ').length;
                        var difference =
                            Math.floor((indentation - state.indentation) / 4) *
                            4;
                        if (difference > 4) difference = 4;
                        var adjustedIndentation =
                            state.indentation + difference;
                        state.indentationDiff =
                            adjustedIndentation - state.indentation;

...
var reg = function reg(pattern) {
    pattern = pattern.replace('<account>', '(@[\\w\\.\\d-]+)').replace('<account-tab>', '(blog|posts|comments|replies|payout|feed|followed|followers|settings|notifications|communities)').replace('<sort>', '(hot|trending|promoted|payout|payout_comments|muted|created)').replace('<tag>', '([\\w\\W\\d-]{1,32})').replace('<permlink>', '([\\w\\d-]+)').replace('/', '\\/');
...
function resolveRoute(path) {
    ...
    if (match) return _GDPRUserList2.default.includes(match[1].substring(1)) ? { page: 'NotFound' } : { page: 'PostsIndex', params: ['home', match[1]] };

    // /@user, /@user/blog, /@user/settings
    match = path.match(routeRegex.UserProfile);
    if (match) return _GDPRUserList2.default.includes(match[1].substring(1)) ? { page: 'NotFound' } : { page: 'UserProfile', params: match.slice(1) };

    // /@user/permlink (redirects to /category/@user/permlink)
    match = path.match(routeRegex.PostNoCategory);
    if (match) return _GDPRUserList2.default.includes(match[1].substring(1)) ? { page: 'NotFound' } : { page: 'PostNoCategory', params: match.slice(1) };

    // /category/@user/permlink
    match = path.match(routeRegex.Post);
    if (match) return _GDPRUserList2.default.includes(match[2].substring(1)) ? { page: 'NotFound' } : { page: 'Post', params: match.slice(1) };

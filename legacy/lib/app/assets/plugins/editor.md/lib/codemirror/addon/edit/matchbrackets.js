...
(function(mod) {
    ...
    function scanForBracket(cm, where, dir, style, config) {
        ...
        var stack = [];
        var re =
            config && config.bracketRegex ? config.bracketRegex : /[(){}[\]]/;
        var lineEnd =
            dir > 0
    ...
    function matchBrackets(cm, autoclear, config) {
        ...
        var maxHighlightLen =
            cm.state.matchBrackets.maxHighlightLineLength || 1000;
        var marks = [],
            ranges = cm.listSelections();
        for (var i = 0; i < ranges.length; i++) {
            var match =
                ranges[i].empty() &&
                findMatchingBracket(cm, ranges[i].head, false, config);
            if (
                match &&
    ...
    function doMatchBrackets(cm) {
        cm.operation(function() {
            ...
            currentlyHighlighted = matchBrackets(
                cm,
                false,

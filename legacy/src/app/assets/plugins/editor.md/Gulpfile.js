...
var headerMiniComment =
    "/*! <%= pkg.name %> v<%= pkg.version %> | <%= fileName(file) %> | <%= pkg.description %> | MIT License | By: <%= pkg.author %> | <%= pkg.homepage %> | <%=pkg.today('Y-m-d') %> */\r\n";
...
var scssTask = function(fileName, path) {
    ...
    return sass(path + fileName + '.scss', {
        style: 'expanded',
        sourcemap: false,
        noCache: true,
    })
...
gulp.task('cm-mode', function() {
    var modes = [codeMirror.path.src.mode + '/meta.js'];

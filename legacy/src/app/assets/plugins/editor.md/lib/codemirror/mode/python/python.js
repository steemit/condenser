    ...
    CodeMirror.defineMode('python', function(conf, parserConf) {
        ...
        if (parserConf.version && parseInt(parserConf.version, 10) == 3) {
            ...
            var singleOperators =
                ...
                new RegExp('^[\\+\\-\\*/%&|\\^~<>!]');
            var identifiers =
                parserConf.identifiers || new RegExp('^[_A-Za-z][_A-Za-z0-9]*');
        }
        ...
        function pushScope(stream, state, type) {
            ...
            offset =
                top(state).offset +
                (type == 'py' ? conf.indentUnit : hangingIndent);
        ...
        function tokenLexer(stream, state) {
            ...
            if (current == '@') {
                if (
                    parserConf.version &&
                    parseInt(parserConf.version, 10) == 3
                ) {
            ...
            var delimiter_index =
                current.length == 1 ? '[({'.indexOf(current) : -1;
        ...
        var external = {
            startState: function(basecolumn) {
                return {
                    tokenize: tokenBase,
                    scopes: [
                        { offset: basecolumn || 0, type: 'py', align: null },
                    ],
                    lastStyle: null,
                    lastToken: null,
                    lambda: false,

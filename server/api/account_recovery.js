...
export default function useAccountRecoveryApi(app) {
    ...
    router.get("/account_recovery_confirmation/:code", function*() {
        ...
        if (!code)
            return this.throw("no confirmation code", 404);
    ...
    router.post("/api/v1/account_identity_providers", koaBody, function*() {
        ...
        try {
            ...
            const { csrf, name, owner_key } = typeof params === "string"
                ? JSON.parse(params)
                : params;
            ...
            if (existing_account) {
                if (existing_account.owner_key === owner_key) {
                    ...
                    this.body = JSON.stringify({
                        status: "found",
                        provider: identity ? identity.provider : null
                    });
                } else {
                    this.body = JSON.stringify({
                        status: "found",

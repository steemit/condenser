...
export default function useGeneralApi(app) {
    ...
    router.post("/accounts", koaBody, function*() {
        ...
        const account = typeof params === "string"
            ? JSON.parse(params)
            : params;
        if (!checkCSRF(this, account.csrf))
            return;
        console.log(
            "-- /accounts -->",
            this.session.uid,
        ...
        try {
            const lock_entity_res = yield Tarantool
                .instance()
                .call("lock_entity", user_id + "");
            ...
            yield new Promise(
                resolve => setTimeout(() => resolve(), rnd_wait_time)
            );
        }
        ...
        try {
            ...
            if (same_ip_bot) {
                ...
                this.body = JSON.stringify({
                    error: "We are sorry, we cannot sign you up at this time because your IP address is associated with bots activity. Please contact support@steemit.com for more information."
                });
                this.status = 401;
                return;
            }
            ...
            const same_ip_account = yield models.Account.findOne({
                ...
                where: { remote_ip: esc(remote_ip) },
                order: "id DESC"
            });
            if (same_ip_account) {
                const minutes = (Date.now() - same_ip_account.created_at) /
                    60000;
                if (minutes < 10) {
                    console.log(
            ...
            if (user.waiting_list) {
                console.log(
                    `api /accounts: waiting_list user ${this.session.uid} #${user_id}`
                );
                throw new Error(
                    "You are on the waiting list. We will get back to you at the earliest possible opportunity."
                );
            }
            ...
            if (!eid) {
                console.log(
                    `api /accounts: not confirmed email for user ${this.session.uid} #${user_id}`
                );
            ...
            const mid = yield models.Identity.findOne({
                attributes: [ "id" ],
                where: { user_id, provider: "phone", verified: true },
                order: "id DESC"
            });
            if (!mid) {
                console.log(
                    `api /accounts: not confirmed sms for user ${this.session.uid} #${user_id}`
                );
            ...
            models.Account
                ...
                .catch(error => {
                    console.error(
                        "!!! Can't create account model in /accounts api",
                        this.session.uid,
                        error
                    );
                });
    ...
    router.post("/update_email", koaBody, function*() {
        if (rateLimitReq(this, this.req))
            return;
        const params = this.request.body;
        const { csrf, email } = typeof params === "string"
            ? JSON.parse(params)
            : params;
    ...
    router.post("/login_account", koaBody, function*() {
        if (rateLimitReq(this, this.req))
            return;
        const params = this.request.body;
        const { csrf, account, signatures } = typeof params === "string"
            ? JSON.parse(params)
            : params;
        if (!checkCSRF(this, csrf))
            return;
        ...
        try {
            ...
            if (signatures) {
                if (!this.session.login_challenge) {
                    ...
                    const [ chainAccount ] = yield Apis.db_api("get_accounts", [
                        account
                    ]);
                    if (!chainAccount) {
                        console.error(
    ...
    router.post("/record_event", koaBody, function*() {
        ...
        try {
            const params = this.request.body;
            const { csrf, type, value } = typeof params === "string"
                ? JSON.parse(params)
                : params;
            if (!checkCSRF(this, csrf))
                return;
            console.log("-- /record_event -->", this.session.uid, type, value);
            const str_value = typeof value === "string"
                ? value
    ...
    router.post("/page_view", koaBody, function*() {
        const params = this.request.body;
        const { csrf, page, ref } = typeof params === "string"
            ? JSON.parse(params)
            : params;
        if (!checkCSRF(this, csrf))
            return;
        ...
        try {
            ...
            if (config.tarantool) {
                const res = yield Tarantool
                    .instance()
                    .call("page_view", page, remote_ip, this.session.uid, ref);
                unique = res[0][0];
            }
            ...
            if (unique) {
                if (page_model) {
                    ...
                    yield (yield models.Page.update({ views }, {
...
function* createAccount(
    ...
    if (!broadcast)
        return signed_transaction.toObject(sx);
    return yield new Promise(
        (resolve, reject) => Apis.broadcastTransaction(sx, () => {
            resolve();
        }).catch(e => {
            reject(e);
        })

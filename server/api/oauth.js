...
function logErrorAndRedirect(ctx, where, error) {
    ...
    if (error.toString())
        ...
        msg = error.error && error.error.message
            ? error.error.message
            : error.msg || JSON.stringify(error);
    console.error(
        `oauth error [${where}|${s.user}|${s.uid}]|${ctx.req.headers["user-agent"]}: ${msg}`
    );
    ...
    try {
        if (this.query["error[error][message]"]) {
            return logErrorAndRedirect(
                this,
                "facebook:1",
                this.query["error[error][message]"]
            );
        }
        ...
        console.log(
            "-- /handle_facebook_callback user id -->",
            this.session.uid,
        ...
        if (this.session.arec) {
            ...
            if (arec) {
                ...
                console.log(
                    "-- /handle_facebook_callback arec -->",
                    this.session.uid,
                    seconds_ago,
                    arec.created_at
                );
                if (seconds_ago < 600)
                    account_recovery_record = arec;
            }
        }
        if (account_recovery_record) {
            if (user) {
                const existing_account = yield models.Account.findOne({
                    ...
                    where: {
                        user_id: user.id,
                        name: account_recovery_record.account_name
                    },
                    order: "id DESC"
                });
                if (existing_account) {
                    console.log(
                        "-- arec: confirmed user for account -->",
                        this.session.uid,
                        provider,
                        account_recovery_record.id,
                        existing_account.name,
                        this.session.uid,
                        account_recovery_record.owner_key
                    );
                    account_recovery_record.update({
                        user_id: user.id,
                        status: "confirmed"
                    });
                    ...
                    console.log(
                        "-- arec: failed to confirm user for account (no account) -->",
                        this.session.uid,
                        provider,
                        account_recovery_record.id,
                        user.id,
                ...
                console.log(
                    "-- arec: failed to confirm user for account (no user) -->",
                    this.session.uid,
                    provider,
                    this.session.uid,
                    this.session.email
        ...
        if (user) {
            ...
            yield models.Identity.update(i_attrs, {
...
function* handleRedditCallback() {
    try {
        ...
        console.log(
            "-- /handle_reddit_callback user id -->",
            this.session.uid,
            user ? user.id : "not found"
        );
        ...
        if (user) {
            if (!waiting_list)
                attrs.waiting_list = false;
            yield models.User.update(attrs, { where: { id: user.id } });
            yield models.Identity.update(i_attrs, {
                where: { user_id: user.id, provider: "reddit" }
            });
            console.log(
                "-- reddit updated user -->",
                this.session.uid,

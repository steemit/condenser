...
var getRates = exports.getRates = function(symbols, cb) {
  ...
  http.get(options, function(res) {
    ...
    res.on('data', function(stuff) {
      ...
      if (parsed.error) {
        return cb(new Error('[0] The result is undefined. Make sure that you are using correct currency symbols'), null);
      }
      var vals = parsed.query.results.rate;
      if (typeof vals == 'object' && vals.Name == 'N/A') {
        return cb(new Error('[1] The result is undefined. Make sure that you are using correct currency symbols'), null);
      } else if (Array.isArray(vals)) {
        for (var i = 0; i < vals.length; i++) {
          if (vals[i].Name == 'N/A') {
            cb(new Error('[2] The result is undefined. Make sure that you are using correct currency symbols'), null);
          }
        }
      }

...
export default class WebSocketClient {
    ...
    */
    constructor(
        ...
        update_rpc_connection_status_callback = null,
        on_reconnect = null,
        update_rpc_request_status_callback = null
    ) {
        ...
        const ws_url = process.env.BROWSER
            ? ws_connection_client
            : ws_connection_server;
        this.connect_promise = new Promise((resolve, reject) => {
            ...
            this.web_socket.onerror = evt => {
                console.error(
                    "ERROR\tWebSocketClient\tconstructor onerror\t",
                    ws_url,
                    evt
                );
                if (this.update_rpc_connection_status_callback)
            ...
            this.web_socket.onmessage = message => {
                ...
                try {
                    ...
                    console.error(
                        "WebSocketClient.onmessage - cannot parse json: ",
                        message.data
                    );
    ...
    */
    request(id, api_id, method, params = [], callback = null) {
        ...
        const request = notice_id != null
            ? {
                id,
                method: "call",
                params: [ api_id, method, [ notice_id, ...params ] ]
            }
        ...
        if (this.socketDebug) {
            ...
            if (reqStr.length > 200)
                reqStr = reqStr.substring(0, 100) + "............." +
                    reqStr.substring(reqStr.length - 100, reqStr.length);
            console.log(
                "WebSocketClient(" + this.instance + ") ----- call " + id +
                    " ---- >",
                reqStr
            );
        }
    ...
    listener(response) {
        if (this.socketDebug) {
            ...
            if (resStr.length > 200)
                resStr = resStr.substring(0, 100) + "............." +
                    resStr.substring(resStr.length - 100, resStr.length);
            console.log(
                "WebSocketClient(" + this.instance + ") <--- reply " +
                    (response.id || " ") +
                    " ---- <",
                resStr
            );
        ...
        if (response.method === "notice") {
            ...
            if (!callback) {
                console.error(
                    "ERROR\tWebSocketClient listener",
                    this.instance,
                    "missing notice_callback id",
                    id,
                    "in response",
                    response
                );
            ...
            try {
                ...
                console.error(
                    "ERROR\tWebSocketClient calling callabck",
                    this.instance,
                    error
                );
            }

...
export function* createTransaction(operations, head, expire_epoc, num) {
    if (head == null || expire_epoc == null || num == null) {
        ...
        const props2 = process.env.BROWSER
            ? (yield select(state => state.global.get("props")))
            : fromJS(props);
        if (head == null) {
        ...
        if (expire_epoc == null) {
            ...
            expire_epoc = Math.ceil(time.getTime() / 1000) +
    ...
    if (!/^[a-f0-9]{40}$/.test(head))
        throw new TypeError(
            "Expecting head block id (40 hex chars), instead got: head = " +
                head
        );

    if (!typeof expire_epoc === "number")
        throw new TypeError(
            "Expecting expire_epoc number, instead got: " + expire_epoc
        );

    if (!typeof num === "number")
        throw new TypeError(
            "Expecting block number, instead got: num = " + num
        );

    const ref_block_num = num & 65535;
    const ref_block_prefix = new Buffer(head, "hex").readUInt32LE(4);
    return {
        ref_block_num,
        ref_block_prefix,
        operations,
        expiration: expire_epoc
    };
}
...
export function signTransaction(tx, keys) {
    if (tx.ref_block_num == null)
        throw new TypeError(
            "Expecting transaction object, instead got tx = " + tx
        );

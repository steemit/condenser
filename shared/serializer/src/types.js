...
Types.asset = {
    fromByteBuffer(b) {
        ...
        let symbol = new Buffer(b_copy.toBinary(), "binary")
            .toString()
            .replace(/\x00/g, "");
        b.skip(7);
    ...
    appendByteBuffer(b, object) {
        ...
        if (!/^[0-9]+\.?[0-9]* [A-Za-z0-9]+$/.test(object))
            throw new Error(
                "Expecting amount like '99.000 SYMBOL', instead got '" +
                    object +
                    "'"
            );
        ...
        if (symbol.length > 6)
            throw new Error(
                "Symbols are not longer than 6 characters " + symbol + "-" +
                    symbol.length
            );

        b.writeInt64(v.to_long(amount.replace(".", "")));
        let dot = amount.indexOf(".");
        // 0.000
...
Types.uint8 = {
    ...
    appendByteBuffer(b, object) {
        v.require_range(0, 255, object, `uint8 ${object}`);
        b.writeUint8(object);
        return;
    },
    fromObject(object) {
...
Types.uint16 = {
    ...
    appendByteBuffer(b, object) {
        v.require_range(0, 65535, object, `uint16 ${object}`);
...
Types.uint32 = {
    ...
    appendByteBuffer(b, object) {
        v.require_range(0, 4294967295, object, `uint32 ${object}`);
...
Types.time_point_sec = {
    ...
    toObject(object, debug = {}) {
        ...
        v.require_range(0, 4294967295, int, `uint32 ${object}`);
...
Types.vote_id = {
    ...
    fromByteBuffer(b) {
        ...
        return { type: value & this.TYPE, id: value & this.ID };
    },
    appendByteBuffer(b, object) {
        v.required(object);
        if (object === "string")
    ...
    fromObject(object) {
        ...
        v.require_range(0, 255, type, `vote type ${object}`);
        v.require_range(0, 16777215, id, `vote id ${object}`);
        return { type, id };
    },
    toObject(object, debug = {}) {
        if (debug.use_default && object === undefined) {
...
Types.static_variant = function(_st_operations) {
    return {
        ...
        toObject(object, debug = {}) {
            if (debug.use_default && object === undefined) {
                return [
                    this.st_operations[0].operation_name,
                    this.st_operations[0].toObject(undefined, debug)
                ];
            }
            v.required(object);
            let type_id = this.opTypeId(object[0]);
    ...
    return {
        validate(array) {
            ...
            for (var i = 0, o; i < array.length; i++) {
                ...
                if (
                    (ref = typeof o[0], [ "number", "string" ].indexOf(ref) >=
                        0)
                ) {
...
Types.public_key = {
    ...
    toObject(object, debug = {}) {
        if (debug.use_default && object === undefined) {
            return ecc_config.address_prefix +
...
let sortOperation = (array, st_operation) => {
    ...
    return st_operation.nosort
        ...
        : st_operation.compare
            ? array.sort((a, b) => st_operation.compare(firstEl(a), firstEl(b)))
            : // custom compare operation
            array.sort(
                (a, b) => typeof firstEl(a) === "number" &&

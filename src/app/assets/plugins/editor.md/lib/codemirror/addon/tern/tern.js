...
(function(mod) {
    ...
    function sendDoc(ts, doc) {
        ts.server.request(
            {
                files: [
                    { type: 'full', name: doc.name, text: docValue(ts, doc) },
                ],
            },
            function(error) {
        ...
        ts.request(
            ...
            function(error, data) {
                ...
                if (ts.options.typeTip) {
                    ...
                    if (data.doc)
                        tip.appendChild(
                            document.createTextNode(' — ' + data.doc)
                        );
                    if (data.url) {
    ...
    function showArgHints(ts, cm, pos) {
        ...
        tip.appendChild(
            document.createTextNode(tp.rettype ? ') ->\u00a0' : ')')
        );
    ...
    function jumpToDef(ts, cm) {
        function inner(varName) {
            ...
            ts.server.request(buildRequest(ts, doc, req), function(
                error,
                data
            ) {
                if (error) return showError(ts, cm, error);
    ...
    function findContext(doc, data) {
        ...
        if (data.start.line == data.end.line)
            var end = Pos(
                nearest.line,
                nearest.ch + (data.end.ch - data.start.ch)
            );
        else
            var end = Pos(
        ...
        ts.request(cm, { type: 'refs' }, function(error, data) {
            ...
            for (var i = 0; i < data.refs.length; i++) {
                ...
                if (ref.file == name) {
                    ...
                    if (
                        cmpPos(cur, ref.start) >= 0 &&
                        cmpPos(cur, ref.end) <= 0
                    )
                        cur = ranges.length - 1;
                }
            }
    ...
    function showError(ts, cm, msg) {
        ...
        else tempTooltip(cm, String(msg));
    }

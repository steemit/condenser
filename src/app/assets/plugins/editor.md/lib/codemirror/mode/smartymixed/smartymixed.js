        ...
        function(config) {
            ...
            var parsers = {
                html: function(stream, state) {
                    var htmlTagName =
                        state.htmlMixedState.htmlState.context &&
                    ...
                    if (
                        !state.inLiteral &&
                        stream.match(regs.htmlHasLeftDelimeter, false) &&
                        htmlTagName === null
                    ) {
                        state.tokenize = parsers.smarty;
                        state.localMode = smartyMode;
                        state.localState = smartyMode.startState(
                            htmlMixedMode.indent(state.htmlMixedState, '')
                        );
                        return helpers.maybeBackup(
                            stream,
                        ...
                        !state.inLiteral &&
                        stream.match(settings.leftDelimiter, false)
                    ) {
                        state.tokenize = parsers.smarty;
                        state.localMode = smartyMode;
                        state.localState = smartyMode.startState(
            ...
            return {
                ...
                copyState: function(state) {
                    ...
                    if (state.localState) {
                        local = CodeMirror.copyState(
                            tok != parsers.html ? smartyMode : htmlMixedMode,
                            state.localState
                        );
                    }
                    return {
                        ...
                        localState: local,
                        htmlMixedState: CodeMirror.copyState(
                            htmlMixedMode,
                            state.htmlMixedState
                        ),
                ...
                token: function(stream, state) {
                    if (stream.match(settings.leftDelimiter, false)) {
                        if (
                            !state.inLiteral &&
                            stream.match(regs.literalOpen, true)
                        ) {
                            state.inLiteral = true;
                            return 'keyword';
                        } else if (
                            state.inLiteral &&
                            stream.match(regs.literalClose, true)
                        ) {

...
(function(mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        ...
        // AMD
        define(['../../lib/codemirror', '../htmlmixed/htmlmixed'], mod); // Plain browser env
    else mod(CodeMirror);
})(function(CodeMirror) {
    'use strict';
            ...
            return {
                ...
                copyState: function(state) {
                    return {
                        ...
                        localState: CodeMirror.copyState(
                            state.localMode,
                            state.localState
                        ),
                    };
                ...
                token: function(stream, state) {
                    ...
                    if (stream.match(/^\/\*/)) {
                        ...
                        if (state.tag == '/' + last(state.kindTag)) {
                            ...
                            state.kindTag.pop();
                            state.localMode =
                                modes[last(state.kind)] || modes.html;
                            state.localState = CodeMirror.startState(
                                state.localMode
                            );
                        }
                ...
                indent: function(state, textAfter) {
                    ...
                    if (top == 'literal') {
                        ...
                        if (/^\s*\{\/(template|deltemplate)\b/.test(textAfter))
                            return 0;
                        if (
                            /^\{(\/|(fallbackmsg|elseif|else|ifempty)\b)/.test(
                                textAfter
                            )
                        )
                            indent -= config.indentUnit;

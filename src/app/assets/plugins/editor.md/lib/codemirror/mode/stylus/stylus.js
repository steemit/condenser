...
(function(mod) {
    ...
    CodeMirror.defineMode('stylus', function(config) {
        var operatorsRegexp = /^(\?:?|\+[+=]?|-[\-=]?|\*[\*=]?|\/=?|[=!:\?]?=|<=?|>=?|%=?|&&|\|=?|\~|!|\^|\\)/,
            ...
            cssValuesWithBracketsRegexp = new RegExp(
                '^(' +
                    cssValuesWithBrackets_.join('|') +
                    ')\\([\\w-\\#\\,\\.\\%\\s\\(\\)]*\\)'
            );
        ...
        var tokenBase = function(stream, state) {
            ...
            if (stream.match(/^\$?[a-z][\w-]+\s?=(\s|[\w-'"\$])/i)) {
                stream.backUp(2);
                var cssPropertie = stream
                    .current()
                    .toLowerCase()
                    .match(/[\w-]+/)[0];
                return cssProperties[cssPropertie] === undefined
                    ? 'variable-2'
                    : 'property';
            } else if (stream.match(/\$[\w-\.]+/i)) {
                return 'variable-2';
            } else if (stream.match(/\$?[\w-]+\.[\w-]+/i)) {
                var cssTypeSelector = stream
                    .current()
                    .toLowerCase()
                    .match(/[\w]+/)[0];
            ...
            if (
                stream.match(operatorsRegexp) ||
                stream.match(wordOperatorsRegexp)
            ) {
                return 'operator';
            }
        ...
        function buildStringTokenizer(quote, greedy) {
            ...
            function stringTokenizer(stream, state) {
                ...
                if (endingString) {
                    ...
                    state.tokenizer = buildInterpolationTokenizer(
                        stringTokenizer
                    );
                    stream.next();
                    return 'operator';
    ...
    var wordOperators = [
        ...
        hintWords = wordOperators.concat(
            commonKeywords,
            commonAtoms,
            commonDef,
            cssTypeSelectors_,

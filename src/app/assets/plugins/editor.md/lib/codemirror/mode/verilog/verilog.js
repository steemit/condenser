...
(function(mod) {
    ...
    CodeMirror.defineMode('verilog', function(config, parserConfig) {
        var indentUnit = config.indentUnit,
            statementIndentUnit =
                parserConfig.statementIndentUnit || indentUnit,
    ...
    function svxGenIndent(stream, state) {
        ...
        switch (state.svxCurCtlFlowChar) {
            ...
            case 'M': // m4
                ...
                break;
            case '@':
                if (state.svxPrevCtlFlowChar == 'S') indentUnitRq = -1; // new pipe stage after stmts
                if (state.svxPrevCtlFlowChar == '|') indentUnitRq = 1; // 1st pipe stage
                break;
            case 'S':
                if (state.svxPrevCtlFlowChar == '@') indentUnitRq = 1; // flow in pipe stage
                if (svxchScopePrefixes[state.svxPrevCtlFlowChar])
                    indentUnitRq = 1; // +1 new scope
                break;
        }
        ...
        hooks: {
            '\\': function(stream, state) {
                ...
                if (stream.sol() && /\\SV/.test(stream.string)) {
                    ...
                    if (
                        (/\\SVX/.test(curPunc) && !state.vxCodeActive) ||
                        (curPunc == '\\SVX_version' && state.vxCodeActive)
                    ) {
                        state.vxCodeActive = true;
                    }
                    style = 'keyword';
                    state.svxCurCtlFlowChar = state.svxPrevPrevCtlFlowChar = state.svxPrevCtlFlowChar =
                        '';
            ...
            tokenBase: function(stream, state) {
                ...
                if (state.vxCodeActive == true) {
                    ...
                    if (
                        state.svxCurCtlFlowChar != vxCurCtlFlowCharValueAtStart
                    ) {
                        // flow change
                        vxIndent = svxGenIndent(stream, state);
                        state.vxIndentRq = vxIndent;

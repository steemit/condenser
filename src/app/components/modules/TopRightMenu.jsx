...
function TopRightMenu({username, showLogin, logout, loggedIn, vertical, navigate, toggleOffCanvasMenu, probablyLoggedIn, nightmodeEnabled, toggleNightmode}) {
    ...
    const submit_story = $STM_Config.read_only_mode ? null : <li className={lcn + ' submit-story' + (vertical ? ' last' : '')}><a href={pathTo.compose()} onClick={nav}>{tt('g.submit_a_story')}</a></li>;
    const submit_icon = $STM_Config.read_only_mode ? null : <li className="show-for-small-only"><Link to={pathTo.compose()}><Icon name="pencil2" /></Link></li>;
    ...
    if (loggedIn) { // change back to if(username) after bug fix:  Clicking on Login does not cause drop-down to close #TEMP!
        ...
        return (
            <ul className={mcn + mcl}>
                <li className={lcn + " Header__search"}><a href={pathTo.search()} title={tt_search}>{vertical ? <span>{tt_search}</span> : <Icon name="search" />}</a></li>
                {submit_story}
    ...
    if (probablyLoggedIn) {
        return (
            <ul className={mcn + mcl}>
                {!vertical && <li className="Header__search"><a href={pathTo.search()} title={tt_search}><Icon name="search" /></a></li>}
    ...
    return (
        <ul className={mcn + mcl}>
            {!vertical && <li className="Header__search"><a href={pathTo.search()} title={tt_search}><Icon name="search" /></a></li>}
            <li className={lcn}><a href={pathTo.signup()}>{tt('g.sign_up')}</a></li>
            <li className={lcn}><a href={pathTo.login()} onClick={showLogin}>{tt('g.login')}</a></li>
            {submit_story}
            {!vertical && submit_icon}
            {toggleOffCanvasMenu && <li className="toggle-menu Header__hamburger"><a href="#" onClick={toggleOffCanvasMenu}>
                <span className="hamburger" />

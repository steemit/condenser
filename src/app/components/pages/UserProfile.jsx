...
export default class UserProfile extends React.Component {
    ...
    render() {
        const SECTION_AUTHOR_REWARDS = 'author-rewards';
        const SECTION_BLOG = 'blog';
        const SECTION_COMMENTS = 'comments';
        const SECTION_CURATION_REWARDS = 'curation-rewards';
        const SECTION_FOLLOWERS = 'followers';
        const SECTION_FOLLOWING = 'followed';
        const SECTION_NOTIFICATIONS = 'notifications';
        const SECTION_PASSWORD = 'password';
        const SECTION_PERMISSIONS = 'permissions';
        const SECTION_RECENT_REPLIES = 'recent-replies';
        const SECTION_SETTINGS = 'settings';
        const SECTION_TRANSFER = 'transfers';
        ...
        if (section === 'posts') section = SECTION_COMMENTS;
        ...
        switch (section) {
            ...
            case SECTION_COMMENTS:
                if (account.post_history) {
                    if (account.comments) {
                        let posts =
                            accountImm.get('posts') ||
                            accountImm.get('comments');
                        if (!fetching && (posts && !posts.size)) {
                            tab_content = (
                                <Callout>
                                    {tt(
                                        'user_profile.user_hasnt_made_any_posts_yet',
                                        { name: accountname }
                                    )}
                                </Callout>
                            );
                ...
                if (account.recent_replies) {
                    ...
                    if (!fetching && (posts && !posts.size)) {
                        tab_content = (
                            <Callout>
                                {tt(
                                    'user_profile.user_hasnt_had_any_replies_yet',
                                    { name: accountname }
                                ) + '.'}
                            </Callout>
                        );
            ...
            case SECTION_BLOG:
                if (account.blog) {
                    ...
                    const emptyText = isMyAccount ? (
                        <div>
                            ...
                            <br />
                            <Link to="/submit.html">Submit a Story</Link>
                            <br />
                            <a href="/steemit/@thecryptofiend/the-missing-faq-a-beginners-guide-to-using-steemit">
                ...
                break;
            default:
                console.log('no matches for section:' + section);
        }

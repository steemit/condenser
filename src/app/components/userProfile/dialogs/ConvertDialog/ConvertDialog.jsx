...
class ConvertDialog extends PureComponent {
    ...
    _getHintText() {
        ...
        switch (type) {
            case TYPES.GOLOS:
                return [
                    'Переведите токены Голоса в Силу Голоса, чтобы увеличить свой вес при голосовании за посты, комментарии и делегатов.',
                    'Силу Голоса нельзя перевести другим пользователям, но можно делегировать. Количество Силы Голоса растет при хранении.',
                    'Делегирование — передача части своей Силы Голоса под управление другому аккаунту. Делегирование можно отменить в любой момент.'
                ];
            case TYPES.POWER:
                return [
                    'Переведите Силу Голоса в токены Голоса, чтобы получить более ликвидный токен, который можно перемещать между пользователями. При этом вы уменьшаете ваш вес при голосовании. Конвертация из Силы Голоса в токены Голоса занимает 13 недель.',
                ];
            case TYPES.GBG:
                return [
                    'Переведите Золотые (GBG) в токены Голоса чтобы получить возможность увеличить свою Силу Голоса или продать их на внешних биржах.',
                    'Конвертация занимает 3,5 дня (?) и ее нельзя отменить после запуска.',
                    'Отсрочка при конвертации Золотого (GBG) в токены Голоса необходима для предотвращения злоупотреблений спекуляцией.'
                ];
        }
    }

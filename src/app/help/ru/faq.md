...
**Лента** - Это самые последние сообщения от людей, на которых вы подписаны.
...
Информация вашего профиля, изображение аватара и фоновая картинка находятся на странице Настройки. Чтобы обновить картинку аватара и фоновую картинку (обложку) вам надо разместить где-либо эти файлы изображений. Можно загрузить их в пост или комментарий на Steemit, либо использовать сервисы для хостинга изображений. Например, [Postimage](http://postimage.io/). После того, как ваша картинка загружена, скопируйте ее URL и вставьте его в поле "Добавьте url вашего изображения" для аватара, либо в поле "URL-адрес изображения обложки" для фонового изображения. Затем нажмите кнопку Обновить и введите ваш пароль или активный ключ.
...
Нет. Такой информации нет на Steemit.com.
...
Вы можете просматривать список файлов на вашем жестком диске, чтобы добавить изображение. Для этого нажмите ссылку выбора изображений в редакторе.
...
Голоса в Steem имеют разный вес, в зависимости от доли токенов, которую имеет голосующий. Поэтому голосующие с большей Силой Голоса (STEEM Power) имеют большее влияние на распределение, чем избиратели с меньшей SP, но их голоса не увеличивают общий размер вознаграждений.
...
Сделки на блокчейне STEEM обычно занимают лишь около трех секунд, но когда вы покупаете токены STEEM с использованием биткоина или некоторых других токенов, то приходится ждать, пока транзакции будут подтверждены в другой сети. Часто это может занять несколько часов, а иногда и дней.
...
Пользователи могут проголосовать против по любой причине, которая у них есть. Множество пользователей в сообществе рекомендуют использовать голосование против лишь для постов, которые являются оскорбительными. Вам решать, следовать ли этим правилам.
...
Единственный способ снизить рейтинг вашей репутации - получить голос против от другого пользователя. Однако не все голоса против будут причиной потери репутации.
...
Да, вы можете использовать кнопку, чтобы поделиться на Facebook, Twitter или LinkedIn. Вы можете разместить ваши Steemit ссылки на других веб-сайтах и сайтах социальных медиа.

<a href="#Table_of_Contents_Followers__Feeds_and_Resteem">^</a>

# Блокчейн

## <span id="What_is_a_blockchain">Что такое блокчейн?</span>

Блокчейн является публичным реестром всех когда-либо совершенных сделок. Все транзакции и данные хранятся в распределенной базе данных. Каждый раз, когда база данных обновляется, все обновления выполняются вместе в одном пакете, который называется «блок». Каждый раз, когда производится/добавляется новый блок, он присоединяется ко всем предыдущим блокам - отсюда и название «blockchain» (цепочка блоков).

<a href="#Table_of_Contents_Blockchain">^</a>

## <span id="What_is_the_Steem_blockchain">Что такое блокчейн Steem?</span>

Блокчейн Steem является публично доступной распределенной базой данных, которая записывает все посты и голосования, а также распределяет вознаграждения в сети. Здесь хранится весь текстовый контент и данные голосования, и тут производятся все расчеты вознаграждений и делаются выплаты.

<a href="#Table_of_Contents_Blockchain">^</a>

## <span id="What_is_the_difference_between_Steem_and_Steemit">В чем разница между Steem и Steemit?</span>

Steem — это имя блокчейна, который хранит все данные и операции и обрабатывает все события, которые происходят. STEEM это также имя для системного токена (валюты).

Steemit это веб-интерфейс для взаимодействия с блокчейном и для просмотра данных блокчейна.

<a href="#Table_of_Contents_Blockchain">^</a>

## <span id="How_is_Steem_different_from_Bitcoin">Чем Steem отличается от биткоина?</span>

На техническом уровне две сети опираются на одну и ту же модель блокчейна, но построены на различных технологиях и кодовой базе. Steem основан на новом уровне развития блокчейна под названием Graphene, который использует «делегатов» вместо «майнеров» для производства блоков.

Модель использования делегатов вместо майнеров, известная как «Делегированное доказательство доли» (Delegated Proof of Stake, Dpos), позволяет повысить эффективность производства блоков. В биткоине (BTC) все 100% создаваемых новых монет распределяются между производителями (майнерами). В блокчейне Steem только 10% новых монет выплачиваются производителям (делегатам). Остальные 90% новых монет STEEM присуждаются производителям контента, кураторам и обладателям Силы Голоса.
...
**Proof of work (Доказательство работы)** - майнеры решают сложную математическую проблему. Майнер, которая решил проблему первым, добавляет блок в блокчейн. За это сеть вознаграждает майнера.

**Proof of stake (Доказательство доли)** - требуется владение криптовалютой (долей). Чем большим количеством токенов вы владеете, тем больше у вас мощности для создания блока. Преимущества: устраняет необходимость в дорогих майнинговых фермах, работает при небольших затратах мощностей, и это требует от производителей блоков иметь долю в токенах сети.

**Delegated proof of stake (Делегированное доказательство доли)** - аккаунты, создающие блоки, коллективно утверждаются владельцами долей в Steem. Вместо того, чтобы полагаться на доказательства работы по поиску блоков, сеть Steem активно распределяет взаимодействие этих аккаунтов так, чтобы удерживать время между блоками в размере 3 секунд.
...
Да. Данные блокчейна можно просматривать различными способами, используя сторониие инструменты, такие как [steemd.com](https://steemd.com) и [steemdb.com](https://steemdb.com).
...
Первоначально запуск Steem произведен 23 марта 2016 года, о чем было объявлено на [Bitcointalk.org](https://bitcointalk.org/index.php?topic=1408726.0). Была найдена ошибка в исходном коде и большинство владельцев долей согласились с тем, что легче ее исправить посредством повторного запуска, чем делать хардфорк. Блокчейн был обнулен и официально вновь заработал 24 марта 2016 года, с оповещением на [Bitcointalk.org](https://bitcointalk.org/index.php?topic=1410943.0).
...
Steemit, Inc. Steemit, Inc. является частной компанией и не доступна для государственных инвестиций.

Хотя это не считается инвестицией, вы можете приобрести токены Steem, которые могут вырасти или упасть в цене. Эти токены можно вложить в Силу Голоса (STEEM Power), которая предоставляет больше влияния на платформе Steem.
...
Бренд и логотип Steemit защищены законами об интеллектуальной собственности, включая авторские права и иные права собственности Соединенных Штатов Америки и других стран. Цель заключается в том, чтобы дать Steemit возможности для защиты бренда и логотипа способами, которые расширяют безопасность пользователя. Никто не вправе допускать несанкционированного коммерческого использования, воспроизводить, создавать производные работы, распространять копии, выполнять или публично демонстрировать логотип или бренд Steemit, за исключением случаев, разрешенных доктриной добросовестного использования, либо по нашему разрешению в письменной форме.

[Раздел 107 Закона об авторском праве](http://www.copyright.gov/title17/92chap1.html#107) обеспечивает нормативную основу для определения, является ли что-то добросовестным использованием и идентифицирует определенные типы использования — например, критику, комментарий, сообщение о новости, обучение, образование и исследование — как примеры мероприятий, которые могут квалифицироваться как добросовестное использование. **Steemit рассматривает художественные вариации логотипа Steemit, которые используются для некоммерческих целей и не используются для причинения вреда пользователям Steemit (например, путем привлечения на фишинговые сайты), как пример добросовестного использования.**

Синий логотип Steem с тремя S-образными фигурами защищен лицензией Creative Commons CC0, что означает возможность свободно копировать, изменять или распространять (даже в коммерческих целях) без необходимости просить разрешения или указывать авторство.
...
## <span id="Did_Steemit__pre_mine__tokens">Намайнил ли Steemit токены заранее?</span>

Токены STEEM, добываемые Steemit, Inc. , не были "намайнены впрок". Вся добыча состоялась после того, как монета была официально и публично объявлена ​​на [Bitcointalk.org](https://bitcointalk.org/index.php?topic=1410943.0).
...
<b>Сохраните ваш главный пароль и держите его в безопасном месте.</b>

Входите в свой аккаунт только с помощью ключа, который имеет соответствующие вашим намерениям разрешения:

- Постинг ключ для повседневного входа
- Активный ключ, когда это необходимо для перевода токенов, повышения силы голоса и т. п.
- Главный пароль или ключ владельца для изменения пароля

Еще раз, **сохраните ваш главный пароль** и держите его в безопасности! Если входите, используя ваш постинг ключ, убедитесь, что при этом не перезапишите и не потеряете ваш главный пароль.

Не рекомендуется вводить свой пароль или ключи на любых сторонних сайтах. Steemit Inc. развивает приложение для авторизации, которое может использоваться на сторонних точках входа в блокчейн Steem.

<a href="#Table_of_Contents_Security">^</a>

## <span id="Why_should_I_be_careful_with_my_master_password">Почему мне нужно быть внимательным с моим главным паролем?</span>

Главный пароль используется для получения всех ключей вашего аккаунта, включая ключ владельца.

<a href="#Table_of_Contents_Security">^</a>

## <span id="Why_is_the_master_password_a_long_string_of_gibberish">Почему главный пароль выглядит длинной тарабарщиной?</span>

Главный пароль должен быть длинным и случайным для достижения максимальной безопасности.

<a href="#Table_of_Contents_Security">^</a>

## <span id="What_are_my_different_keys_for">Для чего мне нужны эти разные ключи?</span>

**Постинг ключ** - позволяет аккаунтам размещать посты, комментировать, редактировать, голосовать, репостить и подписываться на другие аккаунты или отписываться от них. Большинство пользователей ежедневно входят в Steemit с помощью постинг ключа. Чем больше вы используете ваш пароль или ключ, тем больше шансов, что они будут скомпрометированы (доступны посторонним). Поэтому существует ограниченный в возможностях постинг ключ, чтобы уменьшить ущерб, который может причинить скомпрометированный ключ от аккаунта.

**Активный ключ**. Активный ключ предназначен для выполнения более чувствительных задач, таких как передача средств, транзакции по повышению / снижению Силы голоса, конвертация Steem Долларов, голосование за делегатов, обновление данных профиля или аватара, а также размещение ордера на бирже.

**Memo ключ** - в настоящее время Memo ключ не используется.

**Ключ владельца** - ключ владельца предназначен только для использования при крайней необходимости. Это наиболее мощный ключ, потому что он может изменить любой ключ аккаунта включая ключ владельца. В идеале он должен храниться автономно и использоваться только для восстановления скомпрометированного аккаунта.

<a href="#Table_of_Contents_Security">^</a>

## <span id="What_do_I_do_if_I_lost_my_password_keys">Что мне делать, если я потерял свой пароль или ключи?</span>

Не существует способа восстановить свой аккаунт, если вы потеряли ваши пароль или ключ владельца! Поскольку ваш аккаунт имеет реальную ценность, это **очень важно**, сохранить главный пароль в надежном месте, где вы не потеряете его.

Настоятельно рекомендуется хранить офлайновую копию вашего пароля где-то в надежном месте на случай сбоя жесткого диска или других бедствий. Рассмотрите варианты цифрового офлайнового хранилища, такие как внешний диск или флэш-накопитель, а также распечатку на бумаге. Используйте сейф для перестраховки.

<a href="#Table_of_Contents_Security">^</a>

## <span id="Are_my_STEEM_and_Steem_Dollar_tokens_insured_in_the_event_of_a_hack_or_if_someone_takes_over_my_account">Застрахованы ли мои STEEM и Steem Доллары на случай взлома или если кто-то воспользуется моим аккаунтом?</span>

Нет, ликвидные токены не удастся вернуть обратно, если они украдены или отправлены на неправильный аккаунт. Если ваши токены в Силе Голоса (STEEM Power), то хакер не сможет вывести более чем 1/13 часть токенов в неделю. Если ваши токены в сейфе (на сберегательном счету Steem), тогда есть 3 дня, чтобы стало возможным их переводить.

<a href="#Table_of_Contents_Security">^</a>

## <span id="What_should_I_do_if_I_discover_that_someone_hacked_my_account">Что делать, если я узнаю о взломе моего аккаунта?</span>

Если вы создали ваш аккаунт через Steemit и он скомпрометирован, немедленно посетите страницу [Восстановление украденного аккаунта](https://steemit.com/recover_account_step_1). Эта ссылка доступна в основном меню сайта. Вам нужно будет предоставить адрес электронной почты, который вы использовали при создании аккаунта, имя аккаунта и главный пароль, который использовался в течение последних 30 дней.

<a href="#Table_of_Contents_Security">^</a>

## <span id="How_does_the_stolen_account_recovery_process_work">Как происходит восстановление украденного аккаунта?</span>

Если ваш пароль был изменен без вашего согласия, то аккаунт, являющийся вашим аккаунтом восстановления, может сгенерировать новый ключ владельца аккаунта. Восстановление учетной записи должно завершиться в течение 30 дней с момента изменения пароля, и вы должны указать последний ключ владельца, который действовал в течение последних 30 дней.

Steemit Inc. принадлежит аккаунт восстановления (@steem) для всех пользователей, которые регистрируются с помощью Steemit.com. Steemit может идентифицировать пользователей по их оригинальной электронной почте, по логинам Facebook или Reddit, которые использовались для регистрации через Steemit.com.

Если у вас нет главного пароля или ключа владельца, действительных последние 30 дней, или вы не в состоянии доказать, что вы первоначальный владелец аккаунта, то ваш аккаунт не восстановить.
...
Однако разработчикам следует избегать использования термина «Steemit» в своих собственных продуктах, а вместо этого нужно ссылаться на блокчейн Steem или на Steem платформу. Steemit связан со Steemit.com, владельцем которого является Steemit, Inc.
...
Многие разработчики программного обеспечения в настоящее время пользуются преимуществами открытого исходного кода для создания своих приложений на Steem. Таких уже больше шестидесяти.

Этот пост пользователя @fabien содержит больше информации о Steem API: https://steemit.com/steemjs/@fabien/steem-api-now-released

<a href="#Table_of_Contents_Developers">^</a>

## <span id="How_do_I_use_cli_wallet">Как использовать cli_wallet?</span>

Вот руководство пользователя @pfunk, где объясняется, как пользоваться cli_wallet: https://steemit.com/steemhelp/@pfunk/a-learner-s-guide-to-using-steem-s-cliwallet-part-1
...
Блокчейн Steem нужна группа людей для создания блоков и он использует механизм консенсуса, называемый делегированным доказательством доли (delegated proof of stake, DPOS). Сообщество выбирает «делегатов» в качестве производителей блоков и органа управления сетью. Есть 20 постоянно работающих делегатов, которые производят блок каждые 63 секунды. 21-я позиция делится между запасными делегатами, которые выбираются с вероятностью, пропорциональной имеющемуся у них весу голосов от сообщества. Делегаты получают компенсацию в Силе Голоса (STEEM Power) за каждый создаваемый ими блок.

Steemit использует возможности Steem, потому что основатели Steemit считают, что децентрализованная модель хранения и управления текстовым контентом Steem делает этот блокчейн отличной платформой для достижения долгосрочного успеха своей социальной сети и цифровых валютных токенов.
...
Документ Steem Whitepaper (Белые страницы Steem) был создан, чтобы описать механику системы токена, которая делает возможным децентрализованное поощрение и распространение контента таким образом, чтобы улучшить веб-технологии по всем направлениям. Он также применим к Steemit, первому веб-сайту подключенному к блокчейну Steem. Пользователи, прочитавшие Steem Whitepaper, лучше поймут, как их взаимодействие со Steemit является взаимодействием со Steem, с децентрализованной сетью.

Стоит отметить, что документ не обновлялся почти со времени появления Steem. С тех пор было сделано много изменений, поэтому большая часть документа теперь устарела. Он находится в процессе обновления.
...
Если вы разместите свой вопрос в канале [#help](https://steemit.chat/channel/help) на странице чата [steemit.chat](https://steemit.chat/home), его пользователи могут вам помочь.

Вы также можете создать пост на Steemit.com с тегом #help, и кто-то из сообщества может ответить на него.

<a href="#Table_of_Contents_Miscellaneous">^</a>

# <span id="Disclaimer">Отказ от ответственности</span>

## <span id="Third_Party_References_and_User_Links">Упоминания третьих лиц и ссылки пользователей</span>

BlockTrades, Poloniex, Bittrex, Changelly, Shapeshift.io, Coinbase, Localbitcoins, SteemDB, PeerHub, Steemit.chat, SteemTools, SteemCreate, AnonSteem, SteemConnect, Streemian, SteemStats, Pixabay, Steemcleaners, Pexels, Postimage, Markdown Cheatsheet, @cheetah, Bitcointalk, bitcoinfees, blockchain.info и steemd это сторонние приложения/сервисы и они не принадлежат и не поддерживаются Steemit, Inc. Их перечисление здесь, равно как и упоминание любых других сторонних приложений или веб-сайтов, не являются одобрением или рекомендацией от имени Steemit, Inc.

Все ссылки на посты пользователей были созданы нашими пользователями и эти посты не обязательно представляют точку зрения Steemit, Inc. или его администрации. Их перечисление здесь не означает одобрения или рекомендации от имени Steemit, Inc.

Пожалуйста, используйте сторонние сервисы и контент на свой собственный риск.

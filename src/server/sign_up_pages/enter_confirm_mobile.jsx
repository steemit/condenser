...
assets.script.push("/approval_page.js");
...
recovery should your account ever be compromised.</em></p>
    ...
    router.get("/approval", function*() {
        console.log("-- /approval -->", this.session.uid, this.session.user, this.session.whistle_signup);
        let content;
        if (this.query.confirm_email) {
            content = <div>
            ...
            <div className="row">
                <div className="column" style={{maxWidth: '36rem', margin: '0 auto'}}>
                    <div>
                        ...
                        {content}
                        <br />
                        {this.session.whistle_signup ?
                            <button id="close_window" type="button" className="button hollow">
                                DONE
                            </button> : <a href="/" className="button hollow">DONE</a>}
                    </div>
                </div>
            </div>);
        const props = { body, title: "Approval", assets, meta: [] };
        this.body = "<!DOCTYPE html>" +
            renderToString(<ServerHTML {...props} />);
    });

    router.get("/approval_page.js", function*() {
        this.type = 'application/javascript';
        this.body = `
        window.onload = function() {
            document.getElementById('close_window').onClick = function() { window.close(); }
        }
        `;
    });

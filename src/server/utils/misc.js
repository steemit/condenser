...
async function fetchBlockedData(dmca_content_endpoint, dmca_user_endpoint) {
    let blockedUsers = [],
        blockedContents = [];
    try {
        blockedContents = await fetch(dmca_content_endpoint);
        blockedContents = await blockedContents.text();
        blockedContents = blockedContents.split('\n');
    } catch (error) {
        console.error('fetchBlockedData', error);
    }

    try {
        blockedUsers = await fetch(dmca_user_endpoint);
        blockedUsers = await blockedUsers.text();
        blockedUsers = blockedUsers.split('\n');
    } catch (error) {
        console.error('fetchBlockedData', error);
    }

    return { blockedContents, blockedUsers };
}
